...
public class FrameLockListener extends BaseListener {
    ...
    public FrameLockListener(LoUtils plugin, xyz.lokili.loutils.api.IConfigManager configManager) {
        super(plugin, configManager, ConfigConstants.Modules.FRAME_LOCKING, ConfigConstants.FRAME_LOCKING_CONFIG);
    ...
    public void onFrameInteract(PlayerInteractEntityEvent event) {
        ...
        String lockItemName = config.getString("lock-item", "HONEYCOMB");
        ...
        String unlockItemName = config.getString("unlock-item", "BRUSH");
        ...
        if (isLocked) {
            if (config.getBoolean("prevent-rotation", true) || config.getBoolean("prevent-item-removal", true)) {
                player.sendActionBar(Colors.parse(config.getString("messages.is-locked")));
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onFrameDamage(EntityDamageByEntityEvent event) {
        if (!checkEnabled()) return;
        if (!config.getBoolean("prevent-break", true)) return;
