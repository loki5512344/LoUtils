...
public class EnhancedBoneMealListener extends BaseListener {
    ...
    public EnhancedBoneMealListener(LoUtils plugin, xyz.lokili.loutils.api.IConfigManager configManager) {
        super(plugin, configManager, ConfigConstants.Modules.ENHANCED_BONE_MEAL, ConfigConstants.ENHANCED_BONE_MEAL_CONFIG);
    }
    ...
    public void onBoneMealUse(PlayerInteractEvent event) {
        ...
        if (type == Material.SUGAR_CANE && config.getBoolean("sugar-cane.enabled", true)) {
            success = growPlant(block, "sugar-cane");
        } else if (type == Material.CACTUS && config.getBoolean("cactus.enabled", true)) {
    ...
    private boolean growPlant(Block block, String plantType) {
        double chance = config.getDouble(plantType + ".success-chance", 0.75);
        if (random.nextDouble() > chance) return false;

        int maxHeight = config.getInt(plantType + ".max-height", 3);
        int currentHeight = getPlantHeight(block);
        
        if (currentHeight >= maxHeight) return false;

        int minGrowth = config.getInt(plantType + ".growth-amount-min", 1);
        int maxGrowth = config.getInt(plantType + ".growth-amount-max", 3);
        ...
        // Эффекты
        if (config.getBoolean("particles", true)) {
